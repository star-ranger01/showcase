...
with tab1:
    with st.container(height=200, border=200):
        #option = st.multiselect("This dataframe is the raw dataframe. Data consist of Amazon and SOX.", ("Pre-collected data in csv format", "Make an API request for data"))
#        if option == "Pre-collected data in csv format":
#            data()
#            st.dataframe(df)
#
#        else:
#            st.write("Make API call")
        data()
        for k, v in symbol_dict.items():
            st.text(v)
            st.dataframe(df[k], width=700, height=250)
